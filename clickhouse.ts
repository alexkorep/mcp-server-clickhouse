...
// Use http://localhost:2023 for local testing
const CLICKHOUSE_API_BASE_URL =
  process.env.CLICKHOUSE_API_URL || "https://api.clickhouse.cloud";
...
async function callClickHouseApi<T>(
  ...
  try {
    ...
    const errorText = await response.text().catch(() => response.statusText);
    throw new Error(
      `ClickHouse API Error (${method} ${url}): Failed to parse JSON response. Status: ${response.status}. Response text: ${errorText}`
    );
  }
  ...
  if (!response.ok) {
    const errorMessage =
      ...
      response.statusText;
    console.error(
      `ClickHouse API Error Response (${method} ${path}):`,
      responseData
    ); // Log the full error
    throw new Error(
...
const ServicePostRequestSchema = z.object({
  name: z
    .string()
    .describe("Name of the service (alphanumerical, max 50 chars)."),
  ...
  minReplicaMemoryGb: z
    ...
    .multipleOf(4)
    .optional()
    .describe("Min memory/replica (GB, multiple of 4, >=8)."),
  maxReplicaMemoryGb: z
    .number()
    .min(8)
    .multipleOf(4)
    .optional()
    .describe("Max memory/replica (GB, multiple of 4, >=8)."),
  numReplicas: z
    .number()
    .min(1)
    .max(20)
    .optional()
    .describe("Number of replicas (1-20). Defaults vary by tier."),
  ipAccessList: z
    .array(IpAccessListEntrySchema)
    .optional()
    .describe("List of allowed IP addresses."),
...
const ServiceStatePatchRequestSchema = z.object({
  command: z
    .enum(["start", "stop"])
    .describe("Command to change the service state."),
});
const UpdateServiceStateSchema = ServiceIdParamSchema.extend({
  body: ServiceStatePatchRequestSchema.describe(
    "Service state change command."
  ),
});
...
const CLICKHOUSE_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description:
      "Delete a specific service within an organization. The service must be stopped first.",
  ...
  },
  {
    ...
    inputSchema: zodToJsonSchema(
      UpdateServiceStateSchema
    ) as JsonSchema7ObjectType,
  },
  // TODO Add more tools here based on the OpenAPI spec...
];
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (name) {
        ...
        // TODO Add cases for other tools here
